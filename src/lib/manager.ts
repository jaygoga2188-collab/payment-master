...
export async function managerData(selectedDate?: string | null) {
  ...
  const [accounts, cashfreeAccounts, websites, pixels, transactions, logs, metrics, collectionSummary, collectionBySite, collectionByAccount, collectionByWebsiteGateway, gatewayLedger, gatewayLedgerByWebsite] = await Promise.all([
    ...
    sql`SELECT website_id, website_name, account_name, provider, count(*)::int AS payment_count, coalesce(sum(amount_paise), 0)::bigint AS amount_paise FROM (SELECT t.website_id, coalesce(w.name, 'Deleted website') AS website_name, a.account_name, 'razorpay'::text AS provider, t.amount_paise, t.updated_at FROM payment_transactions t LEFT JOIN websites w ON w.id = t.website_id JOIN razorpay_accounts a ON a.id = t.razorpay_account_id WHERE t.status = 'paid' UNION ALL SELECT t.website_id, coalesce(w.name, 'Deleted website') AS website_name, a.account_name, 'cashfree'::text AS provider, t.amount_paise, t.updated_at FROM cashfree_transactions t LEFT JOIN websites w ON w.id = t.website_id JOIN cashfree_accounts a ON a.id = t.cashfree_account_id WHERE t.status = 'paid') paid WHERE (paid.updated_at AT TIME ZONE 'Asia/Kolkata')::date = ${selectedReportDate}::date GROUP BY website_id, website_name, account_name, provider ORDER BY amount_paise DESC, website_name ASC, account_name ASC`,
    ...
    sql`SELECT website_id, website_name, provider, account_name, account_status, archived_at, count(transaction_id)::int AS payment_count, coalesce(sum(amount_paise), 0)::bigint AS amount_paise, max(updated_at) AS last_payment_at FROM (SELECT t.website_id, coalesce(w.name, 'Deleted website') AS website_name, 'razorpay'::text AS provider, a.account_name, CASE WHEN a.archived_at IS NULL THEN a.status ELSE 'archived' END AS account_status, a.archived_at, t.id AS transaction_id, t.amount_paise, t.updated_at FROM payment_transactions t LEFT JOIN websites w ON w.id = t.website_id JOIN razorpay_accounts a ON a.id = t.razorpay_account_id WHERE t.status = 'paid' UNION ALL SELECT t.website_id, coalesce(w.name, 'Deleted website') AS website_name, 'cashfree'::text AS provider, a.account_name, CASE WHEN a.archived_at IS NULL THEN a.status ELSE 'archived' END AS account_status, a.archived_at, t.id AS transaction_id, t.amount_paise, t.updated_at FROM cashfree_transactions t LEFT JOIN websites w ON w.id = t.website_id JOIN cashfree_accounts a ON a.id = t.cashfree_account_id WHERE t.status = 'paid') ledger GROUP BY website_id, website_name, provider, account_name, account_status, archived_at ORDER BY amount_paise DESC, website_name ASC, account_name ASC`,
  ]);
